...
void AUTAD_UI_FPSCharacter::BeginPlay()
{
	...
	if (SkillTree)
	{
		m_pSkillTree = CreateWidget<USkillTree>(GetWorld(), SkillTree);
		m_pSkillTree->AddToViewport();
		m_pSkillTree->CreateTree();
	}
	else
	{
...
void AUTAD_UI_FPSCharacter::SetHealth(int NewHealth)
{
	...
	if (ClampedNewHealth != Health)
	{
		...
		if (Health <= 0)
		{
			if (GameOverWidget)
			{
				GameOverInstance = CreateWidget<UMyFirstUserWidget>(GetWorld(), GameOverWidget);
